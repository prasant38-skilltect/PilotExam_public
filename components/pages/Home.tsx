...
import { Clock, Search, Play, Plane, ExternalLink } from '@/components/Icons';
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (searchTerm.length >= 3) {
      ...
      searchTimeoutRef.current = setTimeout(async () => {
        try {
          const results = await apiRequest('GET', `/api/questions/search?query=${encodeURIComponent(searchTerm)}`);
  ...
  const handleQuizSelect = (quiz: { quizId: number; quizTitle: string; quizSlug: string }) => {
    ...
    window.open(`/quiz/${quiz.quizSlug}`, '_blank');
